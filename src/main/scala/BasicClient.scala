...
case class BasicClient(userId: String, clientType: String="COMPUTER", ip:String="127.0.0.1", port:Int=9173,
  ...
  def getWidgetControlMessages: List[WidgetControl] = messagesReceived.collect{ case wc: WidgetControl => wc }.toList
  def getViewUpdateMessages: List[ViewUp] = messagesReceived.collect{ case vu: ViewUp => vu }.toList
